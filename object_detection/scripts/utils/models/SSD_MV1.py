...
def bbox_predictor(
    layer_in, n_anchors, n_offsets=4, kernel=(3, 3), l2_reg=0.0005, bn=False, dw=False
):
    """Bounding box prediction layer"""
    if kernel == (1, 1):
        layer_out = Conv2D(
            filters=n_anchors * n_offsets,
            kernel_size=kernel,
            strides=(1, 1),
            padding="same",
...
def cls_predictor(
    layer_in, n_anchors, n_classes, kernel=(3, 3), l2_reg=0.0005, bn=False, dw=False
):
    """Category prediction layer"""
    if kernel == (1, 1):
        layer_out = Conv2D(
            filters=n_anchors * (1 + n_classes),
            kernel_size=kernel,
            strides=(1, 1),
            padding="same",
            kernel_initializer="he_normal",
...
def ssd_model(
    ...
    assert cfg.model.input_shape[0] in [
        256,
        224,
        192,
    ], "only the following input sizes are supported[256,224,192] "
    if cfg.model.transfer_learning == True:
        ...
        base_model = tf.keras.applications.MobileNet(
            inp_shape, alpha=cfg.model.model_type.alpha, include_top=False, weights=None
        )
    ...
    if cfg.model.input_shape[0] == 192:
        ...
        anchors_4, cls_preds_4, bbox_preds_4 = fmap_forward(
            fmap_4,
            img_width,
            img_height,
            sizes[4],
        ...
        anchors_3, cls_preds_3, bbox_preds_3 = fmap_forward(
            fmap_3,
            img_width,
            img_height,
            sizes[3],
        ...
        anchors_2, cls_preds_2, bbox_preds_2 = fmap_forward(
            fmap_2,
            img_width,
            img_height,
            sizes[2],
        ...
        anchors_1, cls_preds_1, bbox_preds_1 = fmap_forward(
            fmap_1,
            img_width,
            img_height,
            sizes[1],
        ...
        anchors_0, cls_preds_0, bbox_preds_0 = fmap_forward(
            fmap_0,
            img_width,
            img_height,
            sizes[0],
        ...
        predictions = Concatenate(axis=2, name="predictions")(
            [cls_softmax, bbox_preds, anchors]
        )

        training_model = Model(inputs=base_model.input, outputs=predictions)
        inference_model = Model(
            inputs=base_model.input, outputs=[cls_softmax, bbox_preds, anchors]
        )
        ...
        anchors_5, cls_preds_5, bbox_preds_5 = fmap_forward(
            fmap_5,
            img_width,
            img_height,
            sizes[5],
