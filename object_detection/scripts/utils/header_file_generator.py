...
def gen_h_user_file(config, quantized_model_path):
    ...
    params = Flags(**config)
    if params.post_processing.type == "SSD" and params.model.input_shape == [
        224,
        224,
        3,
    ]:
        _update_ldfile_cproject(
            params.stm32ai.c_project_path, "STM32H747XIHx_CM7_sdram.ld"
        )
    else:
    ...
    with open(os.path.join(path, "ai_model_config.h"), "wt") as f:
        ...
        f.write(
            "#define NO_RESIZE              ({})\n".format(
                int(0 if params.pre_processing.resizing else 1)
            )
        )
        ...
        f.write(
            "#define PP_OFFSET       ({}f)\n".format(
                float(params.pre_processing.rescaling.offset)
            )
        )
        f.write(
            "#define PP_SCALE       ({}f)\n".format(
                float(params.pre_processing.rescaling.scale)
            )
        )
        f.write("\n")
        ...
        opt = [
            "KEEP_ASPECT_RATIO_DISABLE",
            "KEEP_ASPECT_RATIO_CROP",
            "KEEP_ASPECT_RATIO_PADDING",
        ]
        ...
        f.write(
            "#define ASPECT_RATIO_MODE    {}\n".format(
                opt[yaml_opt.index(params.pre_processing.aspect_ratio)]
            )
        )
        f.write("\n\n")
        ...
        if params.post_processing.type == "SSD":
            ...
            raise TypeError(
                "please select one of this supported post processing options [CENTER_NET, YOLO, SSD]"
            )

        if params.post_processing.type == "SSD":
            f.write("\n/* Postprocessing SSD configuration */\n")
            f.write(
                "#define AI_OBJDETECT_SSD_ST_PP_NB_CLASSES         ({})\n".format(
                    len(class_names) + 1
                )
            )
            f.write(
                "#define AI_OBJDETECT_SSD_ST_PP_IOU_THRESHOLD      ({})\n".format(
                    float(params.post_processing.NMS_thresh)
                )
            )
            f.write(
                "#define AI_OBJDETECT_SSD_ST_PP_CONF_THRESHOLD     ({})\n".format(
                    float(params.post_processing.confidence_thresh)
                )
            )
            f.write(
                "#define AI_OBJDETECT_SSD_ST_PP_MAX_BOXES_LIMIT    ({})\n".format(
                    int(params.post_processing.max_boxes_limit)
                )
            )
            f.write(
                "#define AI_OBJDETECT_SSD_ST_PP_TOTAL_DETECTIONS   ({})\n".format(
        ...
        f.write("\n")
        f.write(
            "#define PP_COLOR_MODE    {}\n".format(
                opt[yaml_opt.index(params.pre_processing.color_mode)]
            )
        )
        f.write("\n")
